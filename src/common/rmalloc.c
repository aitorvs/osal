...
static void ControlBlock(begin *B, const char *file)
{
    ...
    if (memcmp(p+B->Size, &End, END_SPACE) != 0) {
        ...
        for (i = 0;   i < END_SPACE;   i++) {
            printf( i%4 ? "%02x" : " %02x", End[i]);
        }

        printf(
                "\n\tis:       ");
        for (i = 0;   i < END_SPACE;   i++) {
            printf( i%sizeof(int) ? "%02x" : " %02x", E[i]);
        }
        printf( "\n"
#ifdef GENERATIONS
                "\tblock was allocated in %s [%u Bytes, generation %u]\n"
...
static void Initialize(void)
{
    ...
    printf(
            ...
            "\tby Aitor Viana <mailto:aitor.viana.sanchez@esa.int>\n"
...
{
    printf( HEAD "Allocating Block with generation %u...\n",
...
void *Rmalloc(size_t size, const char *file)
{
    ...
    if (size == 0) {
        printf( HEAD "WARNING: malloc() demands 0 Bytes (in %s)\n", file);
    }


//    ret = __real_malloc(size+EXTRA_SPACE); /* get extended block  */
...
{
    //    printf("%s\n", __func__);
    return Rmalloc(size, RM_FILE_POS);
}
...
============================================================================= */
static void *Rcalloc(size_t nelem, size_t size, const char *file)
{
    void *ret;
...
============================================================================= */
static void *Rrealloc(void *p, size_t size, const char *file)
{
    void     *ret;
#ifdef WITH_FLAGS
    unsigned  flags = 0;
    ...
    if (p == NULL) {
        ...
        printf( HEAD "Realloc of NULL pointer (in %s)\n", file);
        abort();
...
{
#ifdef ELOQUENT
    printf( HEAD "Free: %p (called from: %s)\n", p, file);
#endif /* ELOQUENT */
    if (p == NULL) {
#ifdef ALLOW_FREE_NULL
#ifdef ELOQUENT
        printf( HEAD "WARNING: Freeing NULL pointer (in %s)\n", file);
#endif
        return;
#else /* !ALLOW_FREE_NULL */
        printf( HEAD "Trying to free NULL pointer (in %s)\n", file);
        abort();
...
#endif
    /* free block */
//    __real_free(((char *)p)-START_SPACE);
...
#define STAT_HEAD "<MALLOC_STATS>\t"
    ...
    if (!Global.BlockCount) {
        printf( STAT_HEAD "Nothing allocated.\n");
    }
    else {
        const begin	**BlockVec;

        if ((BlockVec = (const begin **)malloc(Global.BlockCount*sizeof(begin *))) == NULL) {
            printf( STAT_HEAD "Couldn't allocate enough memory for statistics. Going on...\n");
        }
        else {
            unsigned		 i = 0;
            unsigned		 j;
            ...
            for (i = 0;   i < nrBlocks;   i = j) {
                ...
                for (gen = 0;   gen < count;   gen++) {
                    ...
                    printf( " %d", BlockVec[gen+i]->Generation);
                }
                printf( "\n");
#else
                printf(
            ...
            free(BlockVec);
#ifdef WITH_FLAGS
            printf( STAT_HEAD "*Variable*\t%12u Bytes\n",
                    (unsigned) Mem);
            printf( STAT_HEAD "*Static*  \t%12u Bytes\n",
                    (unsigned) StaticMem);
            printf( STAT_HEAD "*Total*   \t%12u Bytes\n",
                    (unsigned) (Mem+StaticMem));
#else
            printf( STAT_HEAD "*Total*\t%u Bytes\n",
                    (unsigned) Mem);
#endif
        }
    }
    printf( STAT_HEAD "============ END OF STATISTICS =============\n");
#else
    printf( HEAD __FILE__ " not compiled with RM_TEST_DEPTH > 0, call in %s senseless.\n", file);
#endif
}
