...
/* APBUART0 resources (options example only) */
struct rtems_drvmgr_key rastaioN_apbuart0_res[] =
{
	{"txFifoLen", KEY_TYPE_INT, {(unsigned int)32}},
	{"rxFifoLen", KEY_TYPE_INT, {(unsigned int)32}},
	KEY_EMPTY
};
...
{
	{DRIVER_AMBAPP_GAISLER_APBUART_ID, 0, &rastaioN_apbuart0_res[0]},
	{DRIVER_AMBAPP_GAISLER_APBUART_ID, 1, &rastaioN_apbuart0_res[0]}, /* Same resources as for UART0 */
...
struct rtems_drvmgr_drv_res *gr_rasta_io_res[] = 
{
...
void system_init_rastaio(void)
{
	 ...
	 * 
	 */
	gr_rasta_io_set_resources(gr_rasta_io_res, GR_RASTA_IO_NUM);
}
